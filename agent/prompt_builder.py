...
KANBAN_GUIDANCE = (
    ...
    "reviewer=<optional-profile>)`. For an explicitly intra-series patch "
    "review, the reviewer approves without terminalizing the implementation "
    "card by calling `kanban_review_pass`; a final-task reviewer approves with "
    "`kanban_complete`. Either reviewer returns actionable rework with "
